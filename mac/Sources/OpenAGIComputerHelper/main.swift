...
do {
  ...
  if operation == "status" {
    ...
    let screen = ComputerInput.screenRecordingGranted && ComputerInput.displayAvailable && console && !locked
    let input = ComputerInput.accessibilityGranted && console && !locked && !secureInput
    let screenshotReady = screen && ComputerScreenshot.focusedWindowPrivacyReady()
    try writeJSON([
      "screenRecording": screen,
      "accessibility": ComputerInput.accessibilityGranted,
      ...
      "detail": screenshotReady && input ? "ready" : "grant permissions, unlock the console, disable secure input, and focus a window allowed by Capture privacy settings"
    ])
    exit(0)
  }
  ...
  case "click":
    ...
    try ComputerInput.click(x: x, y: y, button: button)
  case "move":
    ...
    try ComputerInput.move(x: x, y: y)
  case "type":
  ...
  case "key":
    ...
    try ComputerInput.sendShortcut(chord)
  case "scroll":
